...
resource "oci_core_instance" "tomcat-server" {
  ...
  create_vnic_details {
    subnet_id        = !var.use_existing_vcn ? oci_core_subnet.vcn01_subnet_app01[0].id : var.compute_subnet_id
    display_name     = "primaryvnic"
    assign_public_ip = false
    nsg_ids          = !var.use_existing_nsg ? [oci_core_network_security_group.SSHSecurityGroup[0].id, oci_core_network_security_group.APPSecurityGroup[0].id] : var.compute_nsg_ids
  }
